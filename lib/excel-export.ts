...
function generateTableRows(items: CheckItem[], obsCount: number): string {
  ...
  items.forEach((item, index) => {
    ...
    if (isFirstInGroup) {
      html += \`
        <td rowspan="\${groupRowSpan}" class="group-obs \${!isObservation ? 'group-op' : ''}">
          \${isObservation ? 'Q.S' : 'V.H'}
        </td>
      \`
    }
...
function escapeHtml(text: string): string {
  if (!text) return ''
  const div = document.createElement ? undefined : null
  if (div) {
